...
export default function Pagination({ type, id, currentPage, totalPages }: {
  type: string, id: string, currentPage?: number | null, totalPages: number
}) {
  ...
  const pages = () => {
    ...
    for (let i = 1; i <= totalPages; i++) {
      pageItems.push(
        <li key={i}>
          <Link scroll={false}  href={`/assets/${id}/${type}/?page=${i}`}
      ...
      ${Number(currentPage) === i ? "bg-gray-700 text-white" : "bg-gray-800"}`}>{i}</Link>
        </li>
      )
    ...
    <nav className="mt-5">
      <ul className="flex items-center -space-x-px h-8 text-sm">
        <li>
          <Link
          scroll={false} 
          href={`/assets/${id}/${type}?page=${Number(currentPage) - 1}`} 
