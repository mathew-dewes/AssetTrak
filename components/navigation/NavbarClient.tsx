    ...
    return (
            ...
            ${openMenu ? "" : "hidden"}`}><span className="sr-only">Menu</span>
                ...
                ${openMenu ? "translate-x-0" : "translate-x-full"}`}>
                    <Link onClick={closeMenu} className={`${path === "/" ? "font-semibold  scale-105" : ""}`} href={'/'}>Home</Link>
                    <Link onClick={closeMenu} className={`${path.startsWith("/assets") ? "font-semibold  scale-105" : ""}`} href={'/assets'}>Assets</Link>
                    <Link onClick={closeMenu} className={`${path.startsWith("/profile") ? "font-semibold  scale-105" : ""}`} href={'/profile'}>Profile</Link>
                    <button className="cursor-pointer flex" onClick={handleSignOut}>Logout</button>
