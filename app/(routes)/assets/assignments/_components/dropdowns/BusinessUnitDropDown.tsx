...
export default function BusinessUnitDropDown({ results }:
  {
    results: {
      unit: BusinessUnit;
      count: number;
  ...
  const updateParam = (type: BusinessUnit) => {
    router.push(`/assets/assignments?bu=${type}`);
    setIsOpen(false)
  }
          ...
          {results.map((item, key) => {
            return <li key={key} onClick={() => updateParam(item.unit)}>
              <div className={`block px-4 py-2 hover:bg-gray-100 ${item.count === 0 ? "pointer-events-none" : "cursor-pointer hover:font-semibold"}`}>
                ...
                <p>{formatCasing(item.unit)} ({item.count})</p>
