...
export default function StatusDropDown({ results }:
  {
    results: {
  ...
  const updateParam = (status: AssignmentStatus) => {
    router.push(`/assets/assignments?action=${status}`);
    setIsOpen(false)
  }
  ...
  return (
    <div className="relative md:inline-block hidden"
      ...
      <button id="dropdownHoverButton"
        ...
        type="button">Action <svg className="w-2.5 h-2.5 ms-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
      ...
      {isOpen && <div id="dropdownHover" className="z-10 bg-white divide-y absolute  divide-gray-100 rounded-lg shadow-sm w-44">
        <ul className="py-2 text-sm text-gray-700 " aria-labelledby="dropdownHoverButton">
          {results.map((item, key) => {
            return <li key={key} onClick={() => updateParam(item.status)}>
